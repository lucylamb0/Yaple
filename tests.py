...
class TestTimePassing(unittest.TestCase):
    def setUp(self):
        user1 = YappleUser("test_user")
        user1.board.current_row = 3
        user1.board.is_complete = True
        user1.board.date = (datetime.date.today() - datetime.timedelta(days=1)).isoformat()
        self.user1 = user1
        self.user1.stats = {
            "games_played": 5,
            "current_streak": 2,
